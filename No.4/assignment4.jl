...
# ╔═╡ 3c6866fd-699c-45cf-b70c-1e422e301c49
using Plots

# ╔═╡ 7b7219f6-90b8-443e-8518-51ab5bb5aa31
using LinearAlgebra

# ╔═╡ 5a2d31c6-6ff9-11ed-058d-11d14d982a12
md"""
# Assignment 4
"""
...
# ╔═╡ a6fa88ca-63b4-49b6-9dbb-964616d29032
...
# ╔═╡ 68688982-6297-40cb-8f3d-c0d7b78f37d4
f(x) = exp(x)

# ╔═╡ 9e60a739-e86b-44f8-b943-d210381575ee
function mclaurin(funcs...)
	order = length(funcs)
	coeffs =  Array{Float64}(undef,order)
	for i in 1:order
		coeffs[i] = funcs[i](0)/factorial(i-1)
	end
	return coeffs
end

# ╔═╡ 81cb972c-b637-4df9-8674-0e14e8135d9e
mclaurin(f,f,f,f,f,f)

# ╔═╡ 1c1e215b-f15d-485d-8f17-ebcb37d6d0e0
function pade(n::Int, m::Int,funcs::Function...)
	N = length(funcs) - 1
	@assert n+m == N "m+n != N"
	c = mclaurin(funcs...)
	if m > 0
		c_high = c[n+2:end]
		matrix = Matrix{Float64}(undef, (m, m))
		for i in 1:size(matrix,1)
			for j in 1:size(matrix,2)
				if n-m+j+i > 0
					matrix[i,j] = c[n-m+j+i]
				else
					matrix[i,j] = 0
				end
			end
		end
		b = reverse(solve_gauss(matrix, -1*c_high))
	
		c_low = c[1:n+1]
		a = c_low[2:end]
		for i in 1:size(a,1)
			a += vcat(zeros(i-1),c_low[1:end-i]) * b[i]
		end
		pushfirst!(a,c[1])
	else 
		a = c
...
# ╔═╡ 8174e27e-50a7-409d-9b46-49f7350d77c9
pade(5,0, f, f,f,f,f,f)

# ╔═╡ e8d746f4-e4dd-4fa4-8584-9af43e4da2ac
pade(2,3, f,f,f,f,f,f)

# ╔═╡ b1fef39c-2bc6-447b-a8a0-adf75b6acd1c
pade(1,4, f, f,f,f,f,f)

# ╔═╡ f062871c-190e-4abc-8c53-a5409e349312
p70 = pade(7,0, f, f,f,f,f,f,f,f)

# ╔═╡ 6134a7fc-9d72-4ca1-bc61-9152b1f4b70c
p34 = pade(3,4, f, f,f,f,f,f,f,f)

# ╔═╡ 44728c0f-720b-4c5f-a75c-179da94f0d46
p25 = pade(2,5, f, f,f,f,f,f,f,f)
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "88bbc8869b89fd681b2343c1799abe268c1dbe3a"
