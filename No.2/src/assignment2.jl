...
f_prime(z::Number) = 3*z*z

# ╔═╡ 59d94a58-93e6-40fd-880d-f1f1b12547c1
...
# ╔═╡ 9558eca4-3f99-4411-8018-8f4bbf873c49
root_newton_raphson(f, f_prime, 0+1im, eps=1e-9)

# ╔═╡ 28ceea10-80d5-438e-bdbb-343665e16392
function plot_roots(func::Function, func_prime::Function, N::StepRangeLen=range(-2,2,401), M::StepRangeLen=N; kwargs...)
	test(x,y) = root_newton_raphson(func, func_prime, x+y*im; kwargs...)
	A = test.(N, M')
	A_1 = angle.(first.(A))
	A_2 = log.(10,last.(A))
	h1 = heatmap(N,M,A_1', c = :viridis, title=L"k(z)")
	h2 = heatmap(N,M,A_2', c = :viridis, title=L" \log(N_{iter})")
	plot(h1, h2, layout=(2,1), size=(1000,1800), xlabel=L"x", ylabel=L"y")
end

# ╔═╡ 4bfa7ee2-2e24-4e3a-96f0-ce6f0a37fd82
begin
anim = @animate for i ∈ -1:10
	plot_roots(f, f_prime,range(-2. ^-i,2. ^-i,1001), eps=1e-9, iter_max=200)
end
gif(anim, "anim.gif", fps=2)
end

# ╔═╡ 56e4b84c-cd1f-4cca-93f3-cd73fa8f3513
g(z) = 35*z^9 - 180*z^7 + 378*z^5 - 420*z^3 + 315*z

# ╔═╡ 3a7b819d-c391-41ca-85e9-853852b8ac93
g_prime(z) = 35*9*z^8 - 180*7*z^6 + 378*4*z^4 - 420*3*z^2 + 315

# ╔═╡ e18153e5-490d-4db7-8e85-79b07fdd117b
plot_roots(g, g_prime, eps=1e-9, iter_max=200)

# ╔═╡ 729f249f-c15c-46f7-a577-8b3cf0323014
begin
c = (0.604,0)
anim2 = @animate for i ∈ -1:23
	plot_roots(g, g_prime,range(first(c)-2. ^-i,first(c)+2. ^-i,401), range(last(c)-2. ^-i,last(c)+2. ^-i,401), eps=1e-9, iter_max=400)
end
