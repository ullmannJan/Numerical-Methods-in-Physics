...
# ╔═╡ 6e44e4f0-e2bd-4afa-a184-2fef7a51be83
md"""
## 5.IV
"""
...
# ╔═╡ 43854c57-265c-4ceb-b936-6c8edfc937e2
function newton_cotes(f::Function, a::Real, b::Real, n::Int)
    h = (b - a) / (n - 1)
	arr = zeros(n)
...
# ╔═╡ 9ec68a49-7d64-4c0f-9066-574e9d5cc9ed
newton_cotes((x -> 2*x), 0., 1., 5)

# ╔═╡ d3e509fd-d3b7-440f-a87e-da955b34d0b5
md"""
### 5.IV.a)
"""

# ╔═╡ a43d456b-e1dc-4e1d-8fb6-fefd05d4a84f
f(x) = x^3 + x

# ╔═╡ 33c5ed38-768c-4bca-803d-16c11fbe10ff
arr = zeros(5)

# ╔═╡ f3b4d0bc-8060-4805-af9e-88d53543a20b
fill_array(arr, (0., 1.), f)
...
# ╔═╡ 73666305-0169-4b29-a4d1-ef12e480d2bf
w_2=4.5

# ╔═╡ b659b1c8-bd06-498e-a2c1-c662930aa219
function prob(x, t; L=L, w_1=w_1, w_2=w_2)
	if 0 < x < L
		psi = (sin(pi*x/L)*exp(-im*w_1*t) + sin(2*pi*x/L)*exp(-im*w_2*t))/sqrt(L) 
		return abs(psi)^2
	end
	return 0
end

# ╔═╡ 19625ee8-5fbc-45e8-bab6-4d650b1cbda6
N, N_e = newton_cotes(x->prob(x, 0.), 0., L, 5)

# ╔═╡ fab40f31-8c44-47d2-9f73-e28f4e04c4ac
newton_cotes(x->prob(x, 0.), 0.75*L, L, 5) ./ N

# ╔═╡ 79d5364b-d9eb-47a3-abfc-023a83dc6d70
ns = 5:2:501

# ╔═╡ ae87d8e4-cd94-43d4-aa62-0b4b0541d89c
t_1 = 0

# ╔═╡ 15bc05ec-f6b2-481e-a327-44bb6859264a
t_2 = pi/(w_2-w_1)

# ╔═╡ 673f450c-18f7-4db2-a64c-eb23c7fbfe35
data = Array{Float64, 2}(undef, size(ns)[1], 2)

# ╔═╡ c112ffec-1f5f-4269-a883-6da30a1cc562
for (i, n) in enumerate(ns)
	data[i, 1] = newton_cotes(x->prob(x, t_1), 0.75*L, L, n)[2]
	data[i, 2] = 0.25*L/(n-1)
end

# ╔═╡ 196d2478-bedc-4ada-82fa-8e14d30da586
begin
	plot(ns, log.(data), label=["log(E)" "log(h)"])
	xlabel!("n")
end

# ╔═╡ 4f9e1d01-ec83-4a96-b6a6-48a53831e203
begin
...
# ╔═╡ 6344bc2d-a06f-49e6-90ab-1a79cf7decf9
data2 = Array{Float64, 2}(undef, size(ns)[1], 2)

# ╔═╡ e59553a1-392e-4155-96dc-d43651cfba7c
for (i, n) in enumerate(ns)
	data2[i, 1] = newton_cotes(x->prob(x, t_2), 0.75*L, L, n)[2]
	data2[i, 2] = 0.25*L/(n-1)
end

# ╔═╡ 9ad8c04e-ff8d-4bd8-8ee5-d8adc726bc0a
begin
	plot(ns, log.(data2), label=["log(E)" "log(h)"])
	xlabel!("n")
end

# ╔═╡ 19cd8902-625f-40a0-8a96-b323e76f7779
begin
	plot(log.(data2[:,2]),log.(data2[:,1]))
	xlabel!("log(h)")
	ylabel!("log(E)")
end
